...
export default function AttendancePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Build query parameters for attendance with search
      ...
      const [attendancesRes, enrollmentsRes, sessionsRes] = await Promise.all([
        api.get('course/attendance/', { params: attendanceParams }),
        api.get('enrollment/?status__iexact=completed'),
        api.get('course/live_session/?')
      ]);
      ...
      // Handle enrollments and sessions data
  ...
  const handleApiError = (error, defaultMessage) => {
    ...
    if (error.response) {
      ...
      if (error.response.status === 400) {
        ...
        // Process all errors and create concise messages
        Object.keys(backendErrors).forEach(key => {
          ...
          // Create concise, user-friendly error messages
          if (key === 'non_field_errors') {
            errorMessages.push(errorValue);
          } else {
            // Just push the error value without repeating the field name
            errorMessages.push(errorValue);
          }
        });
        ...
        if (Object.keys(formErrors).length > 0) {
          ...
          // Show clear error messages in toast
          if (errorMessages.length === 1) {
            // Limit message length for toast but keep it readable
  ...
  const handleChange = (name, value) => {
    ...
    if (name === 'session') {
      ...
      setFormData(prev => ({
        ...prev,
        student: '', // Reset student selection
        [name]: value
      }));
    }
  };
  ...
  // Helper to get enrollment display
  const getEnrollmentDisplay = (id) => {
    const enrollment = enrollments.find(e => e.id === id);
    if (!enrollment) return 'Unknown Enrollment';
    
    const studentName = enrollment.student_data?.full_name || 'Unknown Student';
    const courseTitle = enrollment.timetable_data?.course_detail?.title || 
                        enrollment.timetable_data?.course?.title || 
                        'Unknown Course';
    
    return `${studentName} - Course: ${courseTitle}`;
  };
              ...
              {attendances.length > 0 ? (
                attendances.map((attendance, index) => (
                  <TableRow key={attendance.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>{attendance.student_name || 'Unknown Student'}</TableCell>
                    <TableCell>{attendance.class_title || 'Unknown Class'}</TableCell>
                    <TableCell>{getSessionDisplay(attendance.session)}</TableCell>
                    <TableCell>
                      {attendance.status === 'present' ? (
                        <span className="px-2 py-1 bg-primary/10 text-primary rounded text-xs">Present</span>
                      ) : (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-muted-foreground mb-4">Get started by recording attendance</p>
                      <Button 
                ...
                {currentAttendance && (
                  <div className="space-y-6 py-4">
                    ...
                    <div className="border-b pb-4">
                      ...
                      {currentAttendance.course_data ? (
                        <div className="mt-2 space-y-2 text-sm">
                          <div className="flex justify-between py-2 border-b">
                            <span className="text-muted-foreground">Title:</span>
                            <span className="text-foreground">{currentAttendance.course_data.title}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b">
                            <span className="text-muted-foreground">Description:</span>
                            <span className="text-foreground">{currentAttendance.course_data.description || '-'}</span>
                          </div>
                          <div className="py-2 border-b">
                            <span className="text-muted-foreground">Teachers:</span>
                            <ul className="list-disc pl-5 mt-1">
                              {currentAttendance.course_data.teachers.map(teacher => (
                                <li key={teacher.id} className="text-foreground">{teacher.full_name} ({teacher.email})</li>
                              ))}
                            </ul>
                          </div>
                          <div className="py-2 border-b">
                            ...
                            <ul className="list-disc pl-5 mt-1">
                              {currentAttendance.course_data.subjects.map(subject => (
                                <li key={subject.id} className="text-foreground">{subject.name}</li>
                              ))}
                            </ul>
                          </div>
                          <div className="flex justify-between py-2 border-b">
                            <span className="text-muted-foreground">Capacity:</span>
                            <span className="text-foreground">{currentAttendance.course_data.capacity}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b">
                            <span className="text-muted-foreground">Price:</span>
                            <span className="text-foreground">{formatCurrency(currentAttendance.course_data.price)}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b">
                            <span className="text-muted-foreground">Special Class:</span>
                            <span className="text-foreground">{currentAttendance.course_data.is_special_class ? 'Yes' : 'No'}</span>
                          </div>
                          <div className="py-2 border-b">
                            <span className="text-muted-foreground">Enrolled Students:</span>
                            <ul className="list-disc pl-5 mt-1">
                              {currentAttendance.course_data.enrolled_students.map(student => (
                                <li key={student.id} className="text-foreground">{student.full_name} ({student.email})</li>
                              ))}
                            </ul>
                          </div>
                        </div>
                      ) : (
                        <p className="mt-2 text-muted-foreground">No course data available</p>
                      )}
                    </div>
                    <div className="border-b pb-4">
                      <h3 className="font-medium text-foreground flex items-center gap-2">
                      ...
                      {currentAttendance.session_data ? (
                        <div className="mt-2 space-y-2 text-sm">
                          ...
                          <div className="flex justify-between py-2 border-b">
                            ...
                            <span className="text-foreground">{currentAttendance.session_data.auto_record ? 'Yes' : 'No'}</span>
                          </div>
                        </div>
                      ) : (
