  ...
  async (error) => {
    ...
    if ((status === 401 || status === 403) && !originalRequest._retry) {
      ...
      if (currentRetries >= MAX_RETRY_ATTEMPTS) {
        console.error('Max retry attempts reached for:', requestKey);
        retryAttempts.delete(requestKey);
...
export const quranAPI = {
  ...
  getSurahByNumber: (number) => api.get(`/quran/surahs/${number}/`),
  searchSurahs: (query) => api.get('/quran/surahs/search/', { params: { q: query } }),
  
  // Verses
  getVerses: (params = {}) => api.get('/quran/verses/', { params }),
  searchVerses: (query) => api.get('/quran/verses/search/', { params: { q: query } }),
  
  // Bookmarks
  getBookmarks: (params = {}) => api.get('/quran/bookmarks/', { params }),
  createBookmark: (bookmarkData) => api.post('/quran/bookmarks/', bookmarkData),
  updateBookmark: (id, bookmarkData) => api.patch(`/quran/bookmarks/${id}/`, bookmarkData),
  deleteBookmark: (id) => api.delete(`/quran/bookmarks/${id}/`),
  toggleBookmark: (verseId) => api.post('/quran/bookmarks/toggle/', { verse_id: verseId }),
  
  // Reading History
  getHistory: (params = {}) => api.get('/quran/history/', { params }),
  updateProgress: (surahId, lastVerse) => api.post('/quran/history/update_progress/', { 
    surah_id: surahId, 
...
export const notificationAPI = {
  ...
  markAsRead: (id) => api.post(`/notification/${id}/mark-read/`),
  
  // Mark single notification as unread
  markAsUnread: (id) => api.post(`/notification/${id}/mark-unread/`),
  
  // Mark all notifications as read
  markAllAsRead: () => api.post('/notification/mark-all-read/'),
