...
export const setPendingAction = (action) => {
  try {
    ...
    console.log('✅ Pending action stored:', actionData);
  } catch (error) {
...
export const getPendingAction = () => {
  try {
    ...
    if (diffMinutes > 30) {
      console.log('⚠️ Pending action expired (older than 30 minutes)');
      clearPendingAction();
      return null;
    }
    
    console.log('✅ Retrieved pending action:', action);
    return action;
  } catch (error) {
    console.error('Error retrieving pending action:', error);
    return null;
  }
};
